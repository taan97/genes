<template>
  <div
    ref="bodyRef"
    class="absolute top-0 w-[min(100vw,var(--root-max-width))] h-full overflow-hidden"
  >
    <div
      ref="starRef"
      class="shine"
    />
  </div>
</template>

<script setup>
import { onMounted, ref } from 'vue'

const bodyRef = ref(null)
const starRef = ref(null)
const NUMBERS_OF_STARS = 40
const DELAY = 6
const STAR_SIZE = 30
const ANCHOR_MAX = 100

onMounted(() => {
  Array(NUMBERS_OF_STARS)
    .fill()
    .forEach(() => {
      createStar()
    })
})

function createStar() {
  const star = starRef.value.cloneNode()
  star.style.setProperty('--top', Math.random() * 100 + '%')
  star.style.setProperty('--left', Math.random() * 100 + '%')
  const size = Math.random() * STAR_SIZE
  star.style.setProperty('--star-size', size + 'px')
  star.style.setProperty(
    '--glow-size',
    Math.random() * size * 0.5 + size * 0.3 + 'px'
  )
  star.style.setProperty('--anchor-position', Math.random() * ANCHOR_MAX + 'px')
  star.style.setProperty('--variation', Math.random())
  star.style.webkitAnimationDelay = Math.random() * DELAY + 's'
  star.style.mozAnimationDelay = Math.random() * DELAY + 's'

  bodyRef.value.append(star)
}
</script>

<style scoped>
.shine {
  --star-size: 0px;
  --glow-size: 0px;
  --top: 0px;
  --left: 0px;
  --animation-delay: 0s;
  --anchor-position: 0px;
  --variation: 0px;

  display: block;
  position: absolute;
  top: var(--top);
  left: var(--left);

  width: var(--star-size);
  height: var(--star-size);

  transform-origin: var(--anchor-position);

  background-image: url('data:image/png;base64,');
  background-repeat: no-repeat;
  background-position: center;
  background-size: 100% 100%;
  overflow: hidden;
  color: tansparent;
  -moz-opacity: 0;
  opacity: 0;
  animation: glitter 6s linear var(--animation-delay) infinite normal;
  -webkit-animation: glitter 6s linear var(--animation-delay) infinite normal;
  -moz-animation: glitter 6s linear var(--animation-delay) infinite normal;
  -ms-animation: glitter 6s linear var(--animation-delay) infinite normal;
  -o-animation: glitter 6s linear var(--animation-delay) infinite normal;
  filter: drop-shadow(0px 0px 1rem #fce059);
}

.shine:after {
  content: '';
  position: absolute;
  top: 0;
  bottom: 0;
  left: 0;
  right: 0;
  margin: auto;
  z-index: 1;

  width: var(--glow-size);
  height: var(--glow-size);
  background-color: #fce059;
  border-radius: 50% 50%;
  background-image: radial-gradient(
    circle at center,
    #fff,
    #fce059,
    #fcd046,
    #fcc034,
    #fbaf22
  );
  filter: blur(4px);

  animation: glowing 6s linear var(--animation-delay) infinite normal;
  -webkit-animation: glowing 6s linear var(--animation-delay) infinite normal;
  -moz-animation: glowing 6s linear var(--animation-delay) infinite normal;
  -ms-animation: glowing 6s linear var(--animation-delay) infinite normal;
  -o-animation: glowing 6s linear var(--animation-delay) infinite normal;
}

@-webkit-keyframes glowing {
  0% {
    background-image: radial-gradient(
      circle at center,
      #fff,
      #fce059,
      #fcd046,
      #fcc034,
      #fbaf22
    );
  }
  100% {
    background-image: radial-gradient(circle at center, #fff, #fce059);
  }
}

@-webkit-keyframes glitter {
  0% {
    bottom: -50%;
    left: calc(70% * var(--variation));
    -webkit-transform: rotate(0deg);
    opacity: 1;
  }
  25% {
    -webkit-transform: rotate(10deg);
    opacity: 0.5;
  }
  50% {
    -webkit-transform: rotate(-5deg);
    opacity: 0.8;
  }
  100% {
    top: 0;
    -webkit-transform: rotate(0deg);
    opacity: 0;
  }
}
</style>
