<template>
  <LayoutPromotionGoldBar :setups="layoutSetups">
    ...
    >
      ...
      >
        {{ $t('PagePromotionGoldBar.InputNote') }}
      </p>
      ...
      <i18n-t
        keypath="PagePromotionGoldBar.PageNote"
        tag="p"
...
const layoutSetups = reactive([
  ...
  },
  {
    ...
    handler: () => {
      router.push({ name: 'PagePromotionGoldBarListing' })
    }
  },
  {
...
async function checkQuantity() {
  try {
    await PromotionGoldBarService.canPledge(form.quantity)
