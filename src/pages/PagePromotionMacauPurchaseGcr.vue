<template>
  <BaseHeader class="!bg-black-2 text-white-1">
    <template #sub-header>
      <BaseIcon
        name="logo-orange"
        ...
        >
          ...
          <template #usd>
            <span class="text-orange-1">
              ${{
                $formatCurrency(
                  (route.query.amount * route.query.rate).toFixed(0)
                )
              }}</span
            >
          </template>
  ...
  <button
    type="button"
    class="items-end rounded-full m-5 py-3 px-4 text-center bg-grey-5 text-size-5 text-orange-1 border-gradient-1 mb-10"
    @click="onClickSubmit"
  >
...
async function onClickSubmit() {
  ...
  try {
    await PromotionMacauService.buy({
