<template>
  <BaseHeader :title="$t('PageManagementInformation.SubordinateWithdrawal')">
    <template #sub-header>
  ...
  <div class="p-5">
    <div class="flex flex-col shadow-inner-dark-v-1 rounded-3xl p-4">
      <p class="self-center text-orange-v-1 font-bold text-size-7">
        {{ $t('PageManagementInformation.TotalWithdraw') }}
      </p>
...
const withdrawalListRef = ref(null)

async function doMoreRequested(nextPage, finish) {
  try {
    const res = await UserCenterService.getWithdrawalList({
      startDate: startDate.value,
      endDate: endDate.value,
      pageNumber: nextPage
    })

    totalBalance.value = res.data.total_withdrawal
...
function doChangedStartDate(date) {
  ...
  withdrawalListRef.value.onRefresh()
}
