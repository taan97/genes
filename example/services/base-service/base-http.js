...
import { httpLocaleMap } from '@/locales/languages'
...
class HttpRequest extends HelperService {
   ...
   * @returns {Promise}
   */
  async request(opts = {}, { silent = false, noEncrypt = false } = {}) {
    const config = {
      ...this.config,
      ...opts,
      headers: {
    ...
    if (!noEncrypt && opts.method === 'POST') {
      config.data = this.encrypt(opts.data)
    }

    const response = await this.axios(config)

    if (!silent && opts.method === 'POST' && response.code === 0)
      showToast({
        type: 'success',
  ...
  get config() {
    return {
      ...
      headers: {
        ...
        language: httpLocaleMap[getLocale()],
  ...
  dequeueRequest(requestId) {
    ...
    if (!Object.keys(this.requestQueue).length) {
      ...
      store.setLoadingVisibility(false)
    }
  }
  ...
  queueRequest() {
    ...
    const store = useGlobalStore()
    store.setLoadingVisibility(true)
    return requestId
  }
