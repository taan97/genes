<template>
  <Teleport to="body">
    <Transition name="slide">
      <div
        v-show="isVisible"
        class="fixed top-0 z-[11] bg-black-v-2 h-full w-[min(100vw,var(--root-max-width))] right-[max(0px,calc((100vw-var(--root-max-width))/2))] flex flex-col"
      >
        ...
        <ul class="border-t border-black-v-4 overflow-auto">
          <li
            v-for="item in props.options"
            :key="item[props.keyedBy]"
            class="text-size-3 flex items-center gap-4 px-8 py-5 border-b border-black-v-4 cursor-pointer"
            :class="{
...
function onClickSelected(item) {
  ...
  emit('selected', item)
  props.hideAfterSelection && (isVisible.value = false)
}
</script>
<style scoped>
.slide-enter-active,
